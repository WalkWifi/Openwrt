...
static int startup_mod_init(void)
{
	...
	*(volatile unsigned int *)GPIO1_MODE_ADD  = regbuff;
	...
	startup_gpio_init(GPIO_G3_POWERON, GPIO_DIR_OUT, 0);
	startup_gpio_init(GPIO_G3_POWEROFF, GPIO_DIR_OUT, 1);
	//usleep(300000);
	//startup_gpio_set(GPIO_G3_POWERON, 1);

	printk("Kevin:(drv_startup.ko)(startup_mod_init)| startup option.version(%s)\n",STARTUP_VERSION);
	return 0;
}
...
static void startup_mod_exit(void)
{
	...
	printk("Kevin:(drv_startup.ko)(startup_mod_exit)| startup option.version(%s)\n",STARTUP_VERSION);
}
